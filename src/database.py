...
class InMemoryDB:
    ...
    def get_previous_sg_price(self) -> float | None:
        """Return the stored SG price, or None if not yet set."""
        return self.previous_sg_price

    def set_sg_price(self, price: float):
        """Store the latest SG price, overwriting any previous value."""
        self.previous_sg_price = price
